...
import os
import pandas as pd
from .sd_metrics import compute_surface_distances, compute_robust_hausdorff
import sys
import math
...
def get_LesionWiseScores(prediction_seg, gt_seg, label_value, dil_factor):
    ...
    if np.all(gt_mat==0) and np.all(pred_mat==0):
        ...
        full_sd = compute_surface_distances(gt_mat.astype(int), 
                                                             pred_mat.astype(int), 
                                                             (sx,sy,sz))
        full_hd95 = compute_robust_hausdorff(full_sd, 95)
    ...
    for gtcomp in range(np.max(gt_label_cc)):
        ...
        surface_distances = compute_surface_distances(gt_tmp, pred_tmp, (sx,sy,sz))
        hd = compute_robust_hausdorff(surface_distances, 95)
